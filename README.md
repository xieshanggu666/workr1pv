...
7. **天气防灾**：每天按季节随机生成天气（春雨/暴雨/干旱/暴雪等）并持久化；灾害逐日影响地块生长与动物健康，可投入金币 + 防灾物资购买防护天数免疫损失；连续跳天按天结算防护消耗、损失与恢复

## 数据库表

`player` `plots` `crops` `inventory` `buildings` `animals` `weather_events`

> 天气事件以绝对天数（`abs_day`）为主键唯一持久化，每日结算在事务内完成：读档只是查询、重试只会推进到新的一天，不会重复扣损。
